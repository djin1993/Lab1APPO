...
namespace Game_of_Life
{
	class Game_of_Life
	{
		...
		public static Dictionary<int,object> ReadFromFile(string filename)
		{
		...
		{
			Console.WriteLine ("Enter input and output file {-i <filepath>-for input -o <filepath for output>}");
			string input_filename = Console.ReadLine().Replace("-i ","");
			string output_filename = Console.ReadLine ().Replace ("-o ", "");
			int nr_of_lines = (int)Game_of_Life.ReadFromFile(input_filename)[1];
			int nr_of_cols = (int)Game_of_Life.ReadFromFile(input_filename)[2];
			StreamWriter sw = new StreamWriter(output_filename, true);
			int[,] grid = (int[,])Game_of_Life.ReadFromFile(input_filename)[3];
			...
			Game_of_Life.ReadFromFile(input_filename);
